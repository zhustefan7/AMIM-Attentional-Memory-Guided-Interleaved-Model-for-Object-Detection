...
from network.mvod_bottleneck_lstm_stefan import (
    MobileVOD,
    SSD,
    MobileNetV1,
    MatchPrior,
...
tb_logger = SummaryWriter(log_dir="runs/slowfast")
...
# Params for SGD
parser.add_argument(
    "--lr", "--learning-rate", default=0.0003, type=float, help="initial learning rate"
)
parser.add_argument(
...
parser.add_argument(
    "--sequence_length", default=10, type=int, help="sequence_length of video to unfold"
)
parser.add_argument(
...
if __name__ == "__main__":
    ...
    if args.feature == "mobile_v1":
        ...
        pred_enc_slow = resnet(pretrained=True)
    ...
    if args.freeze_net:
        ...
        for param in pred_enc_fast.parameters():
            param.requires_grad = False
        for param in pred_enc_slow.parameters():
            param.requires_grad = False
    ...
    if args.feature == "fast_slow":
        optimizer = torch.optim.RMSprop(
            [
                # {
                #     "params": [
                #         param for name, param in net.pred_enc_fast.named_parameters()
                #     ],
                #     "lr": base_net_lr,
                # },
                # {
                #     "params": [
                #         param for name, param in net.pred_enc_slow.named_parameters()
                #     ],
                #     "lr": base_net_lr,
                # },
                {
    ...
    for epoch in range(last_epoch + 1, args.num_epochs):
        ...
        if epoch % args.validation_epochs == 0 or epoch == args.num_epochs - 1:
            ...
            model_path = os.path.join(
                output_path, f"WM-slowfast-{args.width_mult}-Epoch-{epoch}.pth"
            )
