...
class VIDDataset:
    ...
    def __init__(self, data, root, transform=None, target_transform=None, is_val=False, batch_size=None, label_file=None):
        ...
        self._name_to_class = {self._classes_map[i]: self._classes_names[i] for i in range(
            len(self._classes_names))}
        self._class_to_ind = {class_name: i for i,
                              class_name in enumerate(self._classes_names)}
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self.load_vid_annotation(
            index) for index in range(0, len(self.seq_list))]
    ...
    def load_vid_annotation(self, i):
        ...
        for image_id in image_ids:
            if self.is_val:
                annotation_file = self.data / \
                    f"Annotations/VID/val/{image_id}.xml"
            else:
                annotation_file = self.data / \
                    f"Annotations/VID/train/{image_id}.xml"
            ...
            for obj in objects:
                ...
                if class_name in self._name_to_class:
                    ...
                    labels.append(
                        self._class_to_ind[self._name_to_class[class_name]])
...
class ImagenetDataset:
    ...
    def __init__(self, data, root, transform=None, target_transform=None, is_val=False, keep_difficult=False, label_file=None):
        ...
        self._name_to_class = {self._classes_map[i]: self._classes_names[i] for i in range(
            len(self._classes_names))}
        self._class_to_ind = {classes_name: i for i,
                              classes_name in enumerate(self._classes_names)}
        self.db = self.gt_roidb()
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self.load_vid_annotation(
            index) for index in range(0, len(self.ids))]
        with open(cache_file, 'wb') as fid:
            pickle.dump(gt_roidb, fid, pickle.HIGHEST_PROTOCOL)
        logging.info('wrote gt roidb to {}'.format(cache_file))
