...
class BottleneckLSTM(nn.Module):
    def __init__(self, input_channels, hidden_channels, height, width, batch_size):
        ...
        (h, c) = self.cell.init_hidden(
            batch_size, hidden=self.hidden_channels, shape=(height, width)
        )
        self.hidden_state = h
        self.cell_state = c
...
class SSD(nn.Module):
    def __init__(
        self, num_classes, batch_size, alpha=1, is_test=False, config=None, device=None
    ):
        """
        Arguments:
            num_classes : an int variable having value of total number of classes
...
class fast_and_slowVOD(nn.Module):
    """
        Module to join encoder and decoder of predictor model
    """

    def __init__(self, pred_enc_fast, pred_enc_slow, pred_dec):
        """
        Arguments:
            pred_enc : an object of MobilenetV1 class
            pred_dec : an object of SSD class
        """
        super(fast_and_slowVOD, self).__init__()
        self.pred_enc_slow = pred_enc_slow
        self.pred_enc_fast = pred_enc_fast
        self.pred_decoder = pred_dec

    def forward(self, seq, index):
        """
        Arguments:
            seq : a tensor used as input to the model  
        Returns:
            confidences and locations of predictions made by model
        """
        # x = self.pred_enc_fast(seq)
        if index % 2 == 0:
            x = self.pred_enc_slow(seq)
        elif index % 2 == 1:
            x = self.pred_enc_fast(seq)
